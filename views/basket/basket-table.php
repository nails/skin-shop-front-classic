				...
				if ( ( ! isset( $no_changes ) || ! $no_changes ) && count( $currencies ) > 1 ) :
					...
					echo '<td colspan="' . $_colspan . '">';
						...
						foreach ( $currencies AS $currency ) :
							...
							if ( $currency->code == SHOP_USER_CURRENCY_CODE ) :
							...
							echo '<option value="' . $currency->code . '" ' . $_selected . '>' . $currency->code . $_symbol . '</option>';

						endforeach;

						echo '</select>';

						if ( SHOP_BASE_CURRENCY_CODE != SHOP_USER_CURRENCY_CODE ) :
